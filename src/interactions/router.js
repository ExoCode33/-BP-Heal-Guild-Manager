...
export async function route(interaction) {
  ...
  if (customId.startsWith('logging_back_')) {
    ...
    const { handleLoggingBackButton } = await import('../services/adminSettings.js');
...
export async function routeSelectMenu(interaction) {
  ...
  if (customId.startsWith('logging_menu_')) {
    ...
    const { handleLoggingMenuSelect } = await import('../services/adminSettings.js');
  ...
  if (customId.startsWith('set_general_log_channel_')) {
    const { handleGeneralChannelSelect } = await import('../services/adminSettings.js');
    return handleGeneralChannelSelect(interaction);
  }

  if (customId.startsWith('set_application_log_channel_')) {
    const { handleApplicationChannelSelect } = await import('../services/adminSettings.js');
    return handleApplicationChannelSelect(interaction);
  }

  // Event toggle with user ID
  if (customId.startsWith('toggle_log_event_')) {
    const { handleEventToggle } = await import('../services/adminSettings.js');
    return handleEventToggle(interaction);
  }
